...
def get_vps_stats(vps):
    ...
    if container_info:
        for line in container_info.split('\n'):
            ...
            if name in ("conduit", "conduit1", "conduit-1"):
                stats["conduit_running"] = is_up
                stats["conduit_uptime"] = uptime_str
                stats["conduit_container"] = name  # Store actual container name
            elif name in ("conduit2", "conduit-2"):
                stats["conduit2_running"] = is_up
                stats["conduit2_uptime"] = uptime_str
                stats["conduit2_container"] = name  # Store actual container name
            elif name == "snowflake":
    ...
    if stats["conduit_running"]:
        container_name = stats.get("conduit_container", "conduit")
        stats_line = docker_command(vps, f"logs --since 10m --tail 100 {container_name} | grep '\\[STATS\\]' | tail -1")
        if stats_line:
            ...
            up_match = re.search(r'Up:\s*([\d.]+)\s*(TB|GB|MB|KB)', stats_line, re.IGNORECASE)
            if up_match:
            ...
            down_match = re.search(r'Down:\s*([\d.]+)\s*(TB|GB|MB|KB)', stats_line, re.IGNORECASE)
            if down_match:
    ...
    if stats["conduit2_running"]:
        container_name2 = stats.get("conduit2_container", "conduit2")
        stats_line2 = docker_command(vps, f"logs --since 10m --tail 100 {container_name2} | grep '\\[STATS\\]' | tail -1")
        if stats_line2:
            ...
            up_match2 = re.search(r'Up:\s*([\d.]+)\s*(TB|GB|MB|KB)', stats_line2, re.IGNORECASE)
            if up_match2:
            ...
            down_match2 = re.search(r'Down:\s*([\d.]+)\s*(TB|GB|MB|KB)', stats_line2, re.IGNORECASE)
            if down_match2:
